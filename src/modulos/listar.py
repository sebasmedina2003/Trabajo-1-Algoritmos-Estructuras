...
def listarProductos(lista: list[str]) -> None:
    ...
    for i in range(0, len(lista)):
        ...
        fechas_creacion.append(datetime.strptime(
            lista[i]["Fecha Creacion"], '%d-%m-%Y').date())
        fechas_modificacion.append(datetime.strptime(
            lista[i]["Fecha Modificacion"], '%d-%m-%Y').date())
    ...
    print("| 6-. Imprimir productos registrados sin criterios                            |")
    print("| 7-. Salir                                                                   |")
    print("+-----------------------------------------------------------------------------+")
    # Captura de la opcion de listado
    try:
        opcion = int(input("-> Opcion deseada: "))
        error = opcion not in range(1, 8)
            ...
            for i in range(len(cantidad)):
                for j in range(len(lista)):
                    if cantidad[i] == lista[j]["Cantidad"]:
                        ...
                        break
            # Imprimir productos ordenados - salida formateada
            mostrarProductos(lista_ordenada)
        elif opcion == 2:
            lista_aux.clear()
            while True:
                ...
                for i in range(len(fechas_creacion)):
                    for j in range(len(lista)):
                        if fechas_creacion[i] == datetime.strptime(lista[j]["Fecha Creacion"], '%d-%m-%Y').date():
                            # print(str(i) + " " + str(lista[j]))
                            # Almacenar en un vector los productos ordenados
                            lista_ordenada.append(lista[j])
                            break
            # Imprimir productos ordenados - salida formateada
            mostrarProductos(lista_ordenada)
            ...
            if not error:
                ...
                if opcion == 1:  # Dimensiones
                    try:
                        ...
                        error = opcion not in range(1, 3)
                        print(
                            "-> Ha ingresado un numero fuera de rango\n" if error else "")
                    except:
                        error = True
                        print(
                            "-> No se ha ingresado un numero, seleccione una opcion valida\n")
                    if not error:
                        ...
                        for i in range(len(dimensiones)):
                            for j in range(len(lista_aux)):
                                ...
                                if dimensiones[i] == (int(dim[0]) * int(dim[1])) and lista_aux[j]["Nombre"] not in reps:
                                    # print(str(i) + " " + str(lista_aux[j]))
                                    # Almacenar los productos en orden en un vector
                                    lista_ordenada.append(lista_aux[j])
                                    reps.append(lista_aux[j]["Nombre"])
                                    break
                    # Mostrar los productos ordenados - salida formateada
                    mostrarProductos(lista_ordenada)
                    ...
                    if not error:
                        ...
                        for i in range(len(fechas_creacion)):
                            for j in range(len(lista)):
                                if fechas_creacion[i] == datetime.strptime(lista[j]["Fecha Creacion"], '%d-%m-%Y').date():
                                    # print(str(i) + " " + str(lista[j]))
                                    # Almacenar los productos ordenados en una lista
                                    lista_ordenada.append(lista[j])
                                    break
                    # Mostrar los productos ordenados - salida formateada
                    mostrarProductos(lista_ordenada)
